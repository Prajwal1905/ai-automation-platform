...
function StatCard({ label, value }) {
  return (
    <div style={{
      ...
      <div style={{ color: "#fff", fontSize: "28px", fontWeight: "600" }}>{value}</div>
    </div>
  )
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/invoices`).then(r => setInvoices(r.data.invoices))
  }, [])
          ...
          <tbody>
            {messages.map((m, i) => (
              <tr key={i} style={{
                ...
                <td style={{ padding: "12px", color: "#aaa", maxWidth: "300px" }}>{m.message}</td>
                <td style={{ padding: "12px" }}><IntentBadge intent={m.intent} /></td>
                <td style={{ padding: "12px", color: m.urgency === "high" ? "#ef4444" : "#aaa" }}>{m.urgency}</td>
          ...
          <tbody>
            {leads.map((l, i) => (
              <tr key={i} style={{
                ...
                <td style={{ padding: "12px", color: "#aaa" }}>{l.email || "--"}</td>
          ...
          <tbody>
            {invoices.map((inv, i) => (
              <tr key={i} style={{
                ...
                <td style={{ padding: "12px" }}>{inv.sender}</td>
                <td style={{ padding: "12px" }}>{inv.vendor}</td>
                <td style={{ padding: "12px", color: "#22c55e" }}>{inv.amount}</td>
                <td style={{ padding: "12px", color: "#aaa" }}>{inv.invoice_number}</td>
                <td style={{ padding: "12px", color: "#aaa" }}>{inv.due_date}</td>
                <td style={{ padding: "12px", color: "#555" }}>
